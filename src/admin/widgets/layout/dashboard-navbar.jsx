...
import { useMaterialTailwindController, setOpenConfigurator, setOpenSidenav } from "admin/context";
import axios from "axios";

export function DashboardNavbar() {
  const [controller, dispatch] = useMaterialTailwindController();
  const { fixedNavbar, openSidenav } = controller;
  const { pathname } = useLocation(); // useLocation is now defined
  const [layout, page] = pathname.split("/").filter((el) => el !== "");
  
  // State quản lý hiển thị Logout button
  ...
  const handleLogout = async () => {
    try {
      ...
      if (response.data.success) {
        // Use navigate to redirect to login page after successful logout
        navigate("/login");
      }
    } catch (error) {
      ...
      <div className="flex flex-col-reverse justify-between gap-6 md:flex-row md:items-center">
        ...
        <div className="flex items-center">
          ...
          <Button
            ...
            onClick={() => setShowLogoutButton(!showLogoutButton)} // Toggle display of Logout button
          >
            <UserCircleIcon className="h-5 w-5 text-blue-gray-500" />
          </Button>
          {/* Toggle button for showing Logout */}
          {showLogoutButton && (
            <Button
              variant="text"
              color="red"
